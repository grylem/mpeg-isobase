...
uint64_t Box::Parser::parse_box(Remainder left, const Box::Map &boxes, const Box::Desc &default_desc) {
    ...
    // lines above
    auto scope = payload_boundary_scope(box);
    ...
    if (left) {
        Blob &blob = stream.peak_blob(left);
        Distance payload(stream);
        notify_about_payload_data(box, blob);
        left.reduce(payload, [&]() {
            return Parser::Exception(SStream() << 
...
void Box::Parser::handle_payload_parsing_exception(const Box &box) {
    try {
        ...
        if (!box.parsing_ctx->ignore_payload_parsing_error) {
            try {
                throw;
            } catch (const bitstream::Parser::Exception &e) {
                throw;  // Just rethrow
            } catch (...) {
                if (box.parsing_ctx->treat_unknow_error_as_parser_one) {
                    try {
                        throw;
                    } catch (const std::runtime_error &e) {
                        throw bitstream::Parser::Exception(SStream() <<
                                "Exception caught while parsing payload of " << box.parsing_ctx->desc.name << ": "<< e.what());
                    } catch (...) {
                        throw bitstream::Parser::Exception(SStream() << 
                                "Unknown exception caught while parsing payload of " << box.parsing_ctx->desc.name);
                    }
                }
            }
        }
