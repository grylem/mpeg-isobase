...
private:
    ...
    struct Boxes {
        std::unordered_map<Box::Type, Box::Desc> types;
        std::unordered_map<std::string, Box::Desc> extended_types;
        ...
        const Box::Desc &get(Box::Type type, const Box::Desc &default_desc) const {
            auto it = types.find(type); return it != types.end() ? it->second: default_desc;
        }
        const Box::Desc &get(const std::string &extended_type, const Box::Desc &default_desc) const {
            auto it = extended_types.find(extended_type); return it != extended_types.end() ? it->second: default_desc;
        }
