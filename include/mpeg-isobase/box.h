...
public:
    bool is_large() const { return _islarge; }
    uint64_t size() const { return _islarge ? largesize: uint64_t(smallsize); }

    void size(uint64_t size) {
        assert(size >= decltype(Box::header)::bytes_occupied);
        assert(bool(_islarge) == bool(size > uint32_t(-1L)));
        if (_islarge) {
            largesize = size;
        } else { 
            smallsize = size;
    ...
    bool is_usertype() const { return (type == 'uuid'); }
...
public:
    ...
    inline Parsing::Control &parsing_control();
...
struct Box::Parsing::Control {
    ...
    bool ignore_payload_parsing_error = false;
    bool treat_unknow_error_as_parser_one = false;
};
...
inline Box::Parsing::Control &Box::parsing_control() { return *parsing_ctx; }
